...
export const validateClasses = (classes, mutate) => {
  if (classes && !mutate) {
    throw new Error(
...
export const validateInherited = ({ classes, inherited }) => {
...
// or not.
export const validateMissing = ({ missing, entries }) => {
  if (missing && !entries) {
    throw new Error(
...
export const validateLeaves = ({ roots, leaves }) => {
  if (roots && leaves) {
    throw new Error(
